...
enum TutorialStepCopy {
    static func text(for step: TutorialStep) -> String {
        switch step {
            ...
            case .afterCameraStripPeekClose: "미리보기 영역 밖을 탭하면 닫을 수 있어요."
            ...
            case .selectionShare: "SNS, 메신저 등으로 공유할 수 있어요."
            case .selectionSave: "기기 앨범 또는 지정한 위치에 저장할 수 있어요."
            case .selectionDelete: "선택한 사진을 삭제할 수 있어요."
    ...
    static func attributedText(for step: TutorialStep) -> AttributedString {
        var attr = AttributedString(text(for: step))
        for keyword in emphasisKeywords(for: step) {
            if let range = attr.range(of: keyword) {
                attr[range].font = .callout.weight(.bold)
    ...
    private static func emphasisKeywords(for step: TutorialStep) -> [String] {
        switch step {
            case .captureGuidePortrait: ["세로"]
            case .captureGuideLeft: ["좌측", "가로"]
            case .captureGuideRight: ["우측", "가로"]
            default: []
        }
    }
}
