...
- Phase: **P6 종료 (P6a · P6b · P6c · P6d 4 그룹 commit 완료)**
- 다음 task: **Phase 7 — Export & Share (P7.1~P7.3)**
- Branch: `feature/ios-mvp/p6d-rewarded-native`
- Last commit: feat(P6d) Rewarded gate + Native ad in Gallery + ATT wiring + roadmap tracking
...
- Done: 선택된 PhotoPair[]를 임시 폴더에 복사 → ZIPFoundation으로 .zip 생성 → `UIActivityViewController` 공유 시트로 제공.

### P7.2 — 기기 저장 (PhotoKit)
- SCOPE: `PairShot/PairShot/Services/PhotoLibraryExport.swift`
- Refs: `.claude/refs/ios-permissions.md`
- Done: PHPhotoLibrary `.addOnly` 권한 → `creationRequestForAssetFromImage` 호출. 권한 거부 시 Settings 딥링크.

### P7.3 — 공유 시트 (`UIActivityViewController`)
- SCOPE: `PairShot/PairShot/Features/Export/ShareSheet.swift`
- Done: SwiftUI `.sheet`에서 UIActivityViewController wrap. 이미지·ZIP 파일 둘 다 처리.
...
- Done: List 기반 섹션 (촬영·합성·내보내기·쿠폰·정보). UserDefaults 백킹.

### P8.2 — JPEG 품질 + 파일명 prefix
- SCOPE: `PairShot/PairShot/Features/Settings/CaptureSettings.swift`
- Done: 품질 0.6/0.8/0.95 3단. 파일명 prefix 텍스트 입력. PhotoStorageService에서 참조.

### P8.3 — Overlay 기본 alpha + 합성 레이아웃 기본값
- SCOPE: `PairShot/PairShot/Features/Settings/CompositionSettings.swift`
- Done: alpha 슬라이더 (After 화면에서 시작값). 합성 레이아웃 horizontal/vertical/grid 3종.

### P8.4 — 저장 용량 표시 + 캐시 정리
- SCOPE: `PairShot/PairShot/Features/Settings/StorageInfo.swift`
- Done: Application Support / photos 폴더 크기 표시. "캐시 정리" → 고아 파일(SwiftData 미참조) 삭제.
